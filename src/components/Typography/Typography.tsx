...
const Typography:FC<TypographyProps> = ({ variant = 'body1', children, color }) => {
    
    switch (variant)
    {
        case 'body1':
            return <p className={`body1 ${color}`} >{children}</p>;
        
        case 'h1':
            return <h1 className={`h1 ${color}`} >{children}</h1>;
        ...
        case 'subtitle1':
            return <h6 className='subtitle1' >{children}</h6>;

        case 'subtitle2':
            return <h6 className={`subtitle2 ${color}`} >{children}</h6>;
